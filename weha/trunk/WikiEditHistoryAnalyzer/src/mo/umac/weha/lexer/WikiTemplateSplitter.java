...
public class WikiTemplateSplitter {
	
	private static final Pattern blockOpener = Pattern.compile("\\{\\{");
	private static final Pattern blockCloser = Pattern.compile("\\}\\}");
	
	private static final Pattern lineTerminator = Pattern.compile("^\\s*\\|", Pattern.MULTILINE);
	
	public static List<Sentence> separateTemplate(int lastPos, String templateText, Paragraph para) {
		...
		while (seekPos < templateText.length()) {
			...
			if (!markupMatchStack.empty() || skipPos <= sentPos) {
				...
				if (ltFound && lastMatch < ltMatcher.start()){
					...
					seekPos = ltMatcher.end();
					lastMatch = seekPos - ltMatcher.group().length();
				}
				else {
					break;
				}
